...
class ColyseusNetwork implements NetworkTransport {
  ...
  constructor(
    events: ConnectionEvents,
    client: NetworkClient = new Client(resolveGameServerUrl(
...
export class CoopNetwork implements NetworkTransport {
  ...
  constructor(
    events: ConnectionEvents,
    modeOrClient: string | NetworkClient = import.meta.env.MODE,
  ) {
    this.#transport = typeof modeOrClient === 'string'
      ? usesHostedTransport(modeOrClient)
        ? new HostedNetwork(events)
        : new ColyseusNetwork(events)
      : new ColyseusNetwork(events, modeOrClient);
  }
  ...
  sendMove(command: MoveTargetCommand): boolean { return this.#transport.sendMove(command); }
